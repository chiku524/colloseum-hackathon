# Creator Team Treasury — Essentials
...
- **Split crank (optional automation)** — Team lead configures on-chain **recipients + bps + interval + max_per_tick**; anyone may call **`crank_automation`** after `next_eligible_ts` to move up to `min(vault, max_per_tick)` split by bps. Projects created before this feature may need **`upgrade_project_layout`** once (Setup tab). Vercel can call **`/api/cron/treasury-crank`** on a schedule when env vars are set (see `apps/web/vercel.json` and `.env.example`).
