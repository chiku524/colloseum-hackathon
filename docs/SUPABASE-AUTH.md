...
- **Verification email never arrives** — Delivery is handled by **Supabase + your custom SMTP** (e.g. Resend), not the Vite app. Check **Auth logs**, **Resend → Emails**, sender on a **verified domain**, and **Redirect URLs** (set **`VITE_AUTH_EMAIL_REDIRECT_ORIGIN`** on Vercel to your canonical site if needed). See **`docs/SUPABASE-CUSTOM-SMTP.md`** (troubleshooting section).
